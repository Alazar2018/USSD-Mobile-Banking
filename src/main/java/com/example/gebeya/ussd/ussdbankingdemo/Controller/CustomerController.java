...
public class CustomerController {
    ...
    public ResponseEntity<String> getAmount(@PathVariable int accountnumber) {
    ...
    @GetMapping("/{cif}")
    public ResponseEntity<?> getCustomerById(@PathVariable int cif) {
...
Alazar Gebeya, [11/23/2023 10:59 PM]
    ...
    public ResponseEntity<String> transferMoney(@RequestBody TransferDTO requestDTO) throws javax.security.auth.login.AccountNotFoundException {
        ...
        MobileBankingUser mobileBankingUser = mobileBankingUserServices.getMobileBankingUserDetailsForCustomer(sender.getCustomer().getCif());
        String verfiypass=mobileBankingUser.getPassword();
        String password=requestDTO.getPassword();
        BigDecimal amount=requestDTO.getAmount();
